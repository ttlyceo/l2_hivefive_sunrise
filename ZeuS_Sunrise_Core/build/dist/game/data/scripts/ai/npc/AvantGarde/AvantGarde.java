...
public class AvantGarde extends AbstractNpcAI
{
		...
		switch (event)
		{
			...
			case "LearnSubClassSkill":
			{
				if (Config.ZEUS_CERTIFICATION_MAIN_AND_SUBCLASS_FULL_SKILL)
				{
					...
					{
						player.sendMessage("You need to have a level greater than or equal to 84");
						return "";
					}
					showSubClassSkillList(player);
					return "";
			...
			case "CancelCertification":
			{
				if (Config.ZEUS_CERTIFICATION_MAIN_AND_SUBCLASS_FULL_SKILL)
				{
					if (player.getAdena() < Config.FEE_DELETE_SUBCLASS_SKILLS)
					{
					...
					boolean updateCert = false;
					Vector<String> NameSkill = new Vector<String>();
					try (Connection con = L2DatabaseFactory.getInstance().getConnection();
						PreparedStatement ps = con.prepareStatement(sql))
					{
						ps.setInt(1, player.getObjectId());
						ps.setInt(2, player.getClassId().getId());
						try (ResultSet rs = ps.executeQuery())
						{
							if (rs.next())
							{
								String SkillToRemove = rs.getString("SkillIds");
								updateCert = true;
								for (String _skill : SkillToRemove.split(","))
								{
									try
									{
										final L2Skill sk = SkillData.getInstance().getInfo(Integer.valueOf(_skill), 1);
										if (sk != null)
										{
											player.removeSkill(sk);
										}
										if (!NameSkill.contains(sk.getName()))
										{
											NameSkill.add(sk.getName());
											player.sendMessage("Removed skill " + sk.getName());
										}
									}
									catch (Exception a)
									{
					...
					{
						_log.warn("Data error on ZeuS Certification " + player.getObjectId() + " : " + e.getMessage(), e);
					}
					...
					if (updateCert)
					{
						String SqlUpdate = "UPDATE zeus_certification set EmergentAbility=?, Skill=?, SkillIds=? WHERE idChar=? AND idClass=? ";
						try (Connection con = L2DatabaseFactory.getInstance().getConnection();
							PreparedStatement ps = con.prepareStatement(SqlUpdate))
						{
							...
							ps.setInt(5, player.getClassId().getId());
							ps.execute();
						}
						catch (Exception e)
						{
							_log.warn("{}: SetEmergent()", player.getObjectId(), e);
						}
					}
	...
	public static void showSubClassSkillList(L2PcInstance player)
	{
		...
		int count = 0;
		boolean hadBlockSkillClass = false;
		List<Integer> BlockSkill = null;
		
		if(Config.ZEUS_CERTIFICACION_CLASSES_SKILL_PROHIBITED!=null){
			if(Config.ZEUS_CERTIFICACION_CLASSES_SKILL_PROHIBITED.containsKey(player.getClassId().getId())){
				hadBlockSkillClass = true;
				BlockSkill = Config.ZEUS_CERTIFICACION_CLASSES_SKILL_PROHIBITED.get(player.getClassId().getId());
				if(BlockSkill==null){
					hadBlockSkillClass = false;
				}else if(BlockSkill.size()==0){
					hadBlockSkillClass = false;
				}
			}
		}
		...
		for (L2SkillLearn s : subClassSkills)
		{
			...
			{
				if(hadBlockSkillClass){
					if(BlockSkill.contains(s.getSkillId())){
						continue;
					}
				}
				count++;
		...
		if (count > 0 || Config.ZEUS_CERTIFICATION_MAIN_AND_SUBCLASS_FULL_SKILL)
		{
