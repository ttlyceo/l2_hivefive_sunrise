...
public class GenerateHtmls
{
	...
	public static void captchaHtml(L2PcInstance activeChar, String botType)
	{
		...
		tb.append("<html><title>防外掛驗證系統</title><body><center>");
		tb.append("請輸入下方的 5 碼驗證碼，並點擊「確認」<br>");
		tb.append("<img src=\"Crest.crest_" + Config.SERVER_ID + "_" + imgId + "\" width=256 height=64><br>");
		tb.append("<table width=254><tr>");
		tb.append("<td width=200 align=left><font color=\"888888\">若圖片無法辨識，請點擊此按鈕重新產生：</font></td><td width=20 align=left");
		...
		switch (botType)
		{
			case "FARM":
				...
				tb.append("<font color=\"888888\">（僅包含英文大寫字母）</font><br1>");
				if (AntibotConfigs.ENABLE_DOUBLE_PROTECTION)
				{
					tb.append("<table width=254><tr>");
					tb.append("<td width=160 align=left><font color=\"849D68\">請選擇正確答案：</font></td>");
				...
				tb.append("<font color=\"FF0000\">剩餘嘗試次數：" + activeChar.getTries() + "</font><br>");
				tb.append("<edit var=\"code\" width=110><br>");
				tb.append("<button value=\"確認\" action=\"bypass -h voice .antibot $code $type\" width=80 height=26 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.button_df\"><br>");

				tb.append("<table bgcolor=2E2E2E><tr><td width=270>");
				tb.append("<font color=898989>若你不小心關閉視窗，可在對話欄輸入 </font>");
				tb.append("<font color=849D68>\".captcha\"</font><font color=898989> 再次打開驗證視窗。<br1>");
				tb.append("你有 " + AntibotConfigs.JAIL_TIMER + " 秒可以完成驗證，最多可輸入 3 次。<br1>");
				tb.append("如果時間到或 3 次皆錯，將會被關入監獄。</font></td>");
			...
			case "ENCHANT":
				...
				if (AntibotConfigs.ENABLE_DOUBLE_PROTECTION)
				{
					...
					tb.append("<td width=160 align=left><font color=\"849D68\">請選擇正確答案：</font></td>");
					tb.append("<td align=center><img src=\"" + _extraImgId + "\" width=32 height=16></td>");
					tb.append("<td align=right><combobox width=62 var=type list=也許;是;否></td>");
					tb.append("</tr></table><br>");
				}
				tb.append("<edit var=\"code\" width=110><br>");
				tb.append("<button value=\"確認\" action=\"bypass -h voice .enchantbot $code $type\" width=80 height=26 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.button_df\"><br>");
				tb.append("<table bgcolor=2E2E2E><tr><td width=270>");
				tb.append("<font color=898989>若你不小心關閉視窗，它會在數秒內自動再次彈出。</font></td>");
