...
public class BoardsManager
{
	public void handleCommands(L2GameClient client, String command)
	{
		...
		if (command.startsWith("_bbsgetfav")) {
			String html = HtmCache.getInstance().getHtm(activeChar.getHtmlPrefix(), "data/html/CommunityBoard/custom_favorite.htm");
			if (html == null) {
				html = "<html><body><br><center>Missing custom_favorite.htm</center></body></html>";
			}
		...
		{
			String htmlFile = "custom_main.htm"; // 預設主頁

			String[] parts = command.split(";");
			if (parts.length >= 2)
			{
				// 轉換成 custom_XXX.htm，例如 custom_shop.htm
				htmlFile = "custom_" + parts[1] + ".htm";
			}

			String path = "data/html/CommunityBoard/" + htmlFile;
			String html = HtmCache.getInstance().getHtm(activeChar.getHtmlPrefix(), path);

			if (html == null || html.isEmpty())
			{
				html = "<html><body><center><font color=FF0000>頁面加載失敗：" + htmlFile + "</font></center></body></html>";
			}

			ShowBoard sb = new ShowBoard(html, "101");
			activeChar.sendPacket(sb);
			activeChar.sendPacket(new ShowBoard(null, "102"));
			activeChar.sendPacket(new ShowBoard(null, "103"));
			return;
		}
		if (command.startsWith("_bbslink")) {	// 3
			command = "_maillist_0_1_0_;DropSearch;0;0;0;0;0;0";
		}
		if (command.startsWith("_bbsloc")) {	// 4
			command = "_maillist_0_1_0_;RaidBossInfo;0;0;0;0;0;0";
		}
		if (command.startsWith("_bbsclan")) {	// 5
			command = "_maillist_0_1_0_;Flagfinder;0;0;0;0;0;0";
			// command = "_maillist_0_1_0_;BugReport;0;0;0;0;0;0";
		}
		if (command.startsWith("_bbsmemo")) {	// 6
			// command = "_maillist_0_1_0_;Flagfinder;0;0;0;0;0;0";
			command = "_maillist_0_1_0_;pvppklog;0;0;0;0;0;0";
		}
		if (command.equals("_maillist_0_1_0_")) {	// 7
			command = "_maillist_0_1_0_;BugReport;0;0;0;0;0;0";
		}
		if (command.startsWith("_friendlist_0_")) {	// 8
			command = "_maillist_0_1_0_;Dressme;0;0;0;0;0;0";
		}
