...
public class OlympiadGameManager implements Runnable
{
	...
	public final void run()
	{
		...
		if (Olympiad.getInstance().inCompPeriod())
		{
			...
			{
				Vector<Integer>RandomArena = new Vector<Integer>();
				...
				for (int i = 0; i < _tasks.length; i++)
				{
					...
					while(!isSelected) {
						if(RandomArena.size() == _tasks.length) {
							RandomArena.clear();
						}
						SelectedStadium = Rnd.get(_tasks.length);
						if(!RandomArena.contains(SelectedStadium)) {
							isSelected = true;
						}
					}
					
					
					task = _tasks[SelectedStadium];
					synchronized (task)
					{
						if (!task.isRunning())
						{
							...
							//team based and classed removed
							/*
							if (((readyClassed != null) || readyTeams) && ((SelectedStadium % 2) == 0))
							{
								// 0,4,8.. arenas checked for teams first
								if (readyTeams && ((SelectedStadium % 4) == 0))
								{
									newGame = OlympiadGameTeams.createGame(SelectedStadium, OlympiadManager.getInstance().getRegisteredTeamsBased());
									if (newGame != null)
									{
								...
								if (readyClassed != null)
								{
									newGame = OlympiadGameClassed.createGame(SelectedStadium, readyClassed);
									if (newGame != null)
									{
										task.attachGame(newGame);
										continue;
							...
							if (readyNonClassed)
							{
								_log.warn("task = _tasks[i];" + task.getZone().getId() + " = " + SelectedStadium);
								newGame = OlympiadGameNonClassed.createGame(SelectedStadium, OlympiadManager.getInstance().getRegisteredNonClassBased());
								if (newGame != null)
								{
									_log.warn("New Game task = _tasks[i];" + task.getZone().getId() + " = " + SelectedStadium);
									task.attachGame(newGame);
									continue;
								}
