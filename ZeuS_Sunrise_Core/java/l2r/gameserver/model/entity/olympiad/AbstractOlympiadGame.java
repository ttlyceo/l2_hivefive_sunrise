...
public abstract class AbstractOlympiadGame
{
	...
	protected static Map<Integer, Integer> PLAYER_INSTANCE = new HashMap<Integer, Integer>();
	...
	protected static final boolean portPlayerToArena(Participant par, Location loc, int id)
	{
		...
		try
		{
			PLAYER_INSTANCE.put(player.getObjectId(), player.getInstanceId());
	...
	protected static final void removals(L2PcInstance player, boolean removeParty)
	{
		try
		{
			...
			if(ClassPathConfig.CLASSPATH_REMOVE_OLY)
				ClassPathsDAO.getInstance().removeSkills(player);
	...
	protected static final void cleanEffects(L2PcInstance player)
	{
		try
		{
			...
			if(ClassPathConfig.CLASSPATH_REMOVE_OLY)
				ClassPathsDAO.getInstance().giveSkills(player);
			player.stopAllEffectsExceptThoseThatLastThroughDeath();
			player.clearSouls();
			player.clearCharges();
			if (player.getAgathionId() > 0)
	...
	protected static final void portPlayerBack(L2PcInstance player)
	{
		...
		Integer value = PLAYER_INSTANCE.get(player.getObjectId());
		if(value == null)
			value = 0;
		player.setInstanceId(value);
		
		if (Config.ENABLE_OLY_ANTIFEED)
		{
			player.startAntifeedProtection(false);
		}
		ZeuS.changeTemplateOly(player, false);
