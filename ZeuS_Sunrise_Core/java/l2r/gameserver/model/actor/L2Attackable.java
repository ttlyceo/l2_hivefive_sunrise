...
public class L2Attackable extends L2Npc
{
				...
				for (DamageDoneInfo reward : rewards.values())
				{
					...
					if (attackerParty == null)
					{
						...
						if (attacker.getKnownList().knowsObject(this))
						{
							...
							final double[] expSp = calculateExpAndSp(levelDiff, damage, totalDamage, attacker.isPremium(), attacker);
							double exp = expSp[0];
							double sp = expSp[1];
					...
					else
					{
						...
						final double[] expSp = calculateExpAndSp(levelDiff, partyDmg, totalDamage, false, lastAttacker == null? null : lastAttacker instanceof L2PcInstance ? lastAttacker.getActingPlayer() : lastAttacker instanceof L2Summon ? ((L2Summon) lastAttacker).getOwner() : null);
						final double[] premiumCheckExpSp = calculateExpAndSp(levelDiff, partyDmg, totalDamage, true, lastAttacker == null? null : lastAttacker instanceof L2PcInstance ? lastAttacker.getActingPlayer() : lastAttacker instanceof L2Summon ? ((L2Summon) lastAttacker).getOwner() : null);
	...
	private ItemHolder calculateRewardItem(L2PcInstance lastAttacker, L2DropData drop, int levelModifier, boolean isSweep)
	{
		...
		if(tmp>0){
			double _tmp = (dropChance * tmp) / 100;
			dropChance+=_tmp;
		}		
		...
		if ((!isRaid() && Config.DEEPBLUE_DROP_RULES) || (isRaid() && Config.DEEPBLUE_DROP_RULES_RAID))
		{
			...
			/*** Need to review this */
			// ZeuS.getChanceItemPremium(lastAttacker, drop.getId(), dropChance);
		}
		...
		long tmp_a = custom_drop_system.getRateToAddPorcent(getId(), drop.getId(), getLevel());
		if(tmp_a>0){
			double _tmp = (itemCount * tmp_a) / 100;
			itemCount+=_tmp;
		}
	...
	private ItemHolder calculateCategorizedRewardItem(L2PcInstance lastAttacker, L2DropCategory categoryDrops, int levelModifier)
	{
		...
		if (Rnd.get(L2DropData.MAX_CHANCE) < categoryDropChance)
		{
			...
			if(tmp>0){
				...
				originalDropChance+=_tmp;
			}
			
			if (getInstanceId() > 0)
			{
				if (getInstanceId() == ZeuS.getVoteInstanceID())
				{
					if (ZeuS.getVoteInstanceDropChance() > 0)
					{
						double porcen = (1000000 * ZeuS.getVoteInstanceDropChance()) / 100;
						originalDropChance += porcen;
					}
					if (ZeuS.getVoteInstanceDropRate() > 0)
					{
						dropRateVoteInstance = ZeuS.getVoteInstanceDropRate();
					}
				}
			}			
			
			double dropChance = originalDropChance * lastAttacker.getRate(Rates.DROP_ITEM, drop.getId(), (isRaid() && !isRaidMinion()));
			...
			if (itemCount > 0)
			{
				L2Object Targ = lastAttacker.getTarget();
				if (Targ instanceof L2Npc)
				{
					int idNpc = ((L2Npc) Targ).getTemplate()._id;
					try
					{
						if ((idNpc >= 18257) && (idNpc <= 18286))
						{
							itemCount = drop.getMaxDrop() == 1 ? 1 : Rnd.get(drop.getMinDrop(), drop.getMaxDrop());
						}
					}
					catch (Exception a)
					{
						_log.warn("Treasure Chest Error ->" + a.getMessage());
					}
				}
				
				if (dropRateVoteInstance > 0)
				{
					itemCount = itemCount + (int) ((itemCount * dropRateVoteInstance) / 100);
				}
	...
	public void doItemDrop(L2NpcTemplate npcTemplate, L2Character mainDamageDealer)
	{
		...
		Vector<Integer> _itemGiven = new Vector<Integer>();
		for (L2DropCategory cat : npcTemplate.getDropData())
		{
			...
			else
			{
				...
				{
					if(_itemGiven!=null) {
						if(_itemGiven.contains(item.getId())) {
							continue;
						}
					}					
	 ...
	 * @return
	 */
	private double[] calculateExpAndSp(int diff, int damage, long totalDamage, boolean isPremium, L2PcInstance attacker)
	{
		double xp;
		double sp;
		...
		xp = ((double) getExpReward(isPremium, attacker) * damage) / totalDamage;
		if (Config.ALT_GAME_EXPONENT_XP != 0)
		{
			xp *= Math.pow(2., -diff / Config.ALT_GAME_EXPONENT_XP);
		}
		
		sp = ((double) getSpReward(isPremium, attacker) * damage) / totalDamage;
	...
	public float getVitalityPoints(int damage)
	{
		...
		final float divider = (getLevel() > 0) && (getExpReward(isPlayer() ? getActingPlayer().isPremium() : false, isPlayer() ? getActingPlayer() : null) > 0) ? (getTemplate().getBaseHpMax() * 9 * getLevel() * getLevel()) / (100 * getExpReward(isPlayer() ? getActingPlayer().isPremium() : false, isPlayer() ? getActingPlayer() : null)) : 0;
		if (divider == 0)
		{
			return 0;
		}
