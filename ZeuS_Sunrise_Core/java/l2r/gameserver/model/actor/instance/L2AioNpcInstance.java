...
public final class L2AioNpcInstance extends L2Npc
{
	...
	public void onBypassFeedback(final L2PcInstance player, String command)
	{
		...
		else if (command.startsWith("Subclass"))
		{
			...
			{
				player.sendMessage("在戰鬥中不能創建或更改副職業。");
				return;
			}
			else if (OlympiadManager.getInstance().isRegistered(player))
			{
				player.sendMessage("當你申請參加奧林匹亞德時，無法變更副職業。");
				return;
			}
			else if (player.isInParty())
			{
				player.sendMessage("當在隊伍中時，副職業無法被創建或更改。");
				return;
			}
			else if (player.isCursedWeaponEquipped())
			{
				player.sendMessage("你無法在裝備詛咒武器時切換副職業！");
				return;
			}
			...
			switch (cmdChoice)
			{
				...
				case 7: // Change Subclass - Action
					...
					else
					{
						...
						player.sendMessage("副職業無法被添加，您已被恢復為您的基礎職業。");
						return;
					}
					break;
			}
		...
		else if (command.startsWith("changename"))
		{
			try
			{
				...
				{
					player.sendMessage("警告，名稱" + val + "已經存在。");
					return;
				}
				...
				player.sendMessage("您的名稱已被更改為" + val);
		...
		else if (command.startsWith("changeclanname"))
		{
			try
			{
				...
				{
					player.sendMessage("警告，血盟名稱" + val + "已經存在。");
					return;
				}
				...
				player.sendMessage("你的血盟名稱已被更改為" + val);
				player.broadcastUserInfo();
			...
			if (Conditions.checkPlayerItemCount(player, itemIdToGet, price))
			{
				...
				if ((parmorInstance != null) && (parmorInstance.getLocationSlot() == armorType))
				{
					...
					if (parmorInstance.isWeapon())
					{
						...
						{
							player.sendMessage("您無法將相同的屬性添加到物品上！");
							return;
						}
					...
					else if (parmorInstance.isArmor())
					{
						if (parmorInstance.getElementals() != null)
						{
							for (Elementals elm : parmorInstance.getElementals())
							{
								...
								{
									player.sendMessage("您無法為物品添加相反的屬性！");
									return;
								}
					...
					player.sendMessage("成功添加" + types[1] + "將屬性添加到您的物品。");
				...
				{
					player.sendMessage("您無法對未裝備的物品進行屬性分配！");
				}
			}
		}
		...
		else if (command.startsWith("teleportTo"))
		{
			try
			{
				...
				{
					player.sendMessage("只有貴族角色可以傳送至該處。");
					return;
				}
		...
		else if (command.startsWith("confirmDonateCode"))
		{
			...
			try
			{
				...
				{
					player.sendMessage("驗證碼錯誤，請再試一次。");
				}
			}
			catch (Exception e)
			{
				player.sendMessage("在添加驗證碼時發生了問題！");
		...
		else if (command.startsWith("itemToAdena"))
		{
			...
			{
				player.sendMessage("你沒有足夠的 " + ItemData.getInstance().getTemplate(AioItemsConfigs.AIO_EXCHANGE_ID).getName() + "。");
			}
