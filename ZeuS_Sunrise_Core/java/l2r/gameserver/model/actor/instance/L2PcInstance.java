...
public class L2PcInstance extends L2Playable
{
		...
		if (isEquiped)
		{
			if(item.getItem().getBodyPart() == L2Item.SLOT_L_BRACELET){
                if(this.isMounted() || this.isTransformed()){
                    //this.dismount();
                	this.sendMessage("請在卸下此物品前先解除裝備/變形。");
                	return;
                }
            }
	...
	public int giveAvailableSkills(boolean includedByFs, boolean includeAutoGet)
	{
		...
		{
			sendMessage("你已學會" + skillCounter + "新技能");
		}
		return skillCounter;
	}
	...
	public void standUp()
	{
		...
		{
			sendMessage("當你是增益效果販售者時，你不能使用這個指令。");
			return;
		}
		if (SunriseEvents.isInEvent(this) && getEventInfo().isSitForced())
		{
			sendMessage("當你試圖站起來時，一股超越你凡人理解的黑暗力量使你的膝蓋顫抖...");
			return;
		}
		try{
	...
	public void doAttack(L2Character target)
	{
		...
		if (target instanceof L2PcInstance) // isPKProtected((L2PcInstance) target))
		{
			...
			{
				sendMessage("您無法攻擊等級過低或處於離開狀態的玩家。");
				sendPacket(ActionFailed.STATIC_PACKET);
			}
			else if(ZeuS.isBorrowActice(this))
			{
				sendMessage("您無法攻擊使用借用帳號的玩家。");
				sendPacket(ActionFailed.STATIC_PACKET);				
			}
			else
			{
	...
	public boolean canOpenPrivateStore()
	{
		...
		{
			sendMessage("無法在活動進行或已註冊的狀態下開啟商店。");
			return false;
		}
		...
		else
		{
			...
			if (ptarget != null)
			{
				if (answer == 1)
				{
					...
					ptarget.sendMessage("要求進行交戰已被>接受<");
				}
				else
				{
					ptarget.sendMessage("要求進行交戰已被>拒絕<!");
				}
			}
		}
	}
	...
	public void onPlayerEnter()
	{
        ...
        if (SevenSigns.getInstance().isSealValidationPeriod() || SevenSigns.getInstance().isCompResultsPeriod())
		{
			if (!isGM() && isIn7sDungeon() && (SevenSigns.getInstance().getPlayerCabal(getObjectId()) != SevenSigns.getInstance().getCabalHighestScore()))
			{
				...
				sendMessage("由於封印驗證期的開始，您已被傳送到最近的城鎮。");
			}
		}
		else
		{
			if (!isGM() && isIn7sDungeon() && (SevenSigns.getInstance().getPlayerCabal(getObjectId()) == SevenSigns.CABAL_NULL))
			{
				teleToLocation(TeleportWhereType.TOWN);
				setIsIn7sDungeon(false);
				sendMessage("因為您尚未加入任何陣營，所以您已被傳送至最近的城鎮。");
			}
		}
		...
		if (isGM())
		{
			if (isInvul())
			{
				sendMessage("進入世界以無敵模式。");
			}
			if (isInvisible())
			{
				sendMessage("進入世界以隱形模式。");
			}
			if (isSilenceMode())
			{
				sendMessage("進入世界時處於寂靜模式。");
			}
		}
