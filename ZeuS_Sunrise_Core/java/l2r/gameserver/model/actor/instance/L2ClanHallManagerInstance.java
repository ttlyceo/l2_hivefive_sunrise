		...
		if (condition == COND_OWNER)
		{
			...
			else if (actualCommand.equalsIgnoreCase("door"))
			{
				...
				if (player.hasClanPrivilege(ClanPrivilege.CH_OPEN_DOOR))
				{
					if (val.equalsIgnoreCase("open"))
					{
						...
						CreatureSay cs = null;
						for(L2Character pppl : temp.getZone().getCharactersInside()) {
							if(pppl.isNpc()) {
								if(pppl.getClass().getSimpleName().indexOf("ClanHallManager")>0) {
									cs = new CreatureSay(pppl.getObjectId(), 0, pppl.getName(), "Secure Chat are Disable. All close player can read you (All Chat)");
									break;
								}
							}
						}
					...
					else if (val.equalsIgnoreCase("close"))
					{
						...
						for(L2Character pppl : temp.getZone().getCharactersInside()) {
							if(pppl.isNpc()) {
								if(pppl.getClass().getSimpleName().indexOf("ClanHallManager")>0) {
									cs = new CreatureSay(pppl.getObjectId(), 0, pppl.getName(), "Secure Chat are Enable. Only Player inside the Clan Hall can Read you (All Chat)");
									break;
								}
							}
						}
				...
				if (player.hasClanPrivilege(ClanPrivilege.CH_SET_FUNCTIONS))
				{
					if (val.equalsIgnoreCase("recovery"))
					{
						if (st.countTokens() >= 1)
						{
							...
							{
								player.sendMessage("這個血盟大廳沒有擁有者，你無法更改配置。");
								return;
							}
							val = st.nextToken();
			...
			else if (actualCommand.equalsIgnoreCase("support"))
			{
				if (player.isCursedWeaponEquipped())
				{
					...
					player.sendMessage("持有被詛咒武器的玩家無法接受外部治療或增益效果。");
					return;
				}
				setTarget(player);
				L2Skill skill;
				...
				try
				{
					...
					{
						player.sendMessage("技能等級無效，請聯繫您的管理員！");
					}
				}
				catch (Exception e)
				{
