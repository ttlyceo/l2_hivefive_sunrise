...
public class L2DoormenInstance extends L2NpcInstance
{
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		else if(command.startsWith("clone_me"))
		{
			if(isOwnerClan(player)) {
				if(player.getClan().getLeaderId() == player.getObjectId()) {
					fakenpc.getInstance().createFakeClone(player, getTemplate()._id, true);
				}
			}
			return;
		}
		else if(command.startsWith("remove_clone"))
		{
			if(isOwnerClan(player)) {
				if(player.getClan().getLeaderId() == player.getObjectId()) {
					fakenpc.getInstance().resetFake(player, getTemplate()._id);
				}
			}
			return;
		}
