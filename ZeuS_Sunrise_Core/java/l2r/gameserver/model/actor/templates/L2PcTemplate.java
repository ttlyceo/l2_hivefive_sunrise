...
public class L2PcTemplate extends L2CharTemplate
{
	...
	public float getBaseHpMax(int level)
	{
		if (level < 0)
			return _baseHp[0];
		if (level >= _baseHp.length)
			return _baseHp[_baseHp.length - 1];
	...
	public float getBaseMpMax(int level)
	{
		if (level < 0)
			return _baseMp[0];
		if (level >= _baseMp.length)
			return _baseMp[_baseMp.length - 1];
	...
	public float getBaseCpMax(int level)
	{
		if (level < 0)
			return _baseCp[0];
		if (level >= _baseCp.length)
			return _baseCp[_baseCp.length - 1];
	...
	public double getBaseHpRegen(int level)
	{
		if (level < 0)
			return _baseHpReg[0];
		if (level >= _baseHpReg.length)
			return _baseHpReg[_baseHpReg.length - 1];
	...
	public double getBaseMpRegen(int level)
	{
		if (level < 0)
			return _baseMpReg[0];
		if (level >= _baseMpReg.length)
			return _baseMpReg[_baseMpReg.length - 1];
	...
	public double getBaseCpRegen(int level)
	{
		if (level < 0)
			return _baseCpReg[0];
		if (level >= _baseCpReg.length)
			return _baseCpReg[_baseCpReg.length - 1];
