...
public final class L2NpcTemplate extends L2CharTemplate implements IIdentifiable
{
	...
	public void generateChanceDrops(){
        ...
        for (L2DropCategory category : _categories) {
            ...
            try
            {
                dropListScope = Enum.valueOf(DropListScope.class, category.getCategoryType() > 0? "DEATH" : "CORPSE");
            }
            catch (Exception e)
            {
            }
            ...
            if (dropListScope != null)
            {
                ...
                GroupedGeneralDropItem dropItem = dropListScope.newGroupedDropItem(category.getCategoryChance()); // probably need to be reworked

                List<IDropItem> groupedDropList = new ArrayList<>(2);


                for (L2DropData allDrop : category.getAllDrops()) {
                    final IDropItem dropItem2 = dropListScope.newDropItem(allDrop.getId(), allDrop.getMinDrop(), allDrop.getMaxDrop(), ((allDrop.getChance() / 1000000)*100));
                    if(dropItem2 != null)
                        groupedDropList.add(dropItem2);
                }
                ...
                for (IDropItem item : groupedDropList)
                {
                    ...
                    {
                        System.out.println("[{}] grouped general drop item supports only general drop item.");
                    }
                }
	...
	{
		return getAIDataStatic().getClanRange();
	}
