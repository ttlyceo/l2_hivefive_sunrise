...
public final class L2ItemInstance extends L2Object
{
	...
	{
		setOwnerId(owner_id);
		if (Config.LOG_ITEMS && (creator != null))
		{
	...
	public void setZeuSBidNewOwner(int owner_id)
	{
		zeusBidNewOwner = owner_id;
	}
	
	public int getZeuSNewOwner()
	{
		return zeusBidNewOwner;
	}
	...
	{
		if ((this.getOwnerId() == -300) || (this.getOwnerId() == -301))
		{
			return;
		}
	...
	private void removeFromDb()
	{
		...
		if ((getOwnerId() == -300) || (getOwnerId() == -301))
		{
			return;
		}
		...
		boolean removefromBD = true;
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			try (PreparedStatement ps = con.prepareStatement("DELETE FROM items WHERE object_id = ? and owner_id != -300 AND owner_id != -301"))
			{
				ps.setInt(1, getObjectId());
