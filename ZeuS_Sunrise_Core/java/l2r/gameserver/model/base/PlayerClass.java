...
public enum PlayerClass
{
	...
	static
	{
		Set<PlayerClass> subclasses = getSet(null, ( ZeuS.subClassCumulativeIsEnabled() ? Fourth : Third ) );
	...
	public final Set<PlayerClass> getAvailableSubclasses(L2PcInstance player)
	{
		...
		ClassLevel tempLevel = null;
		
		if(ZeuS.subClassCumulativeIsEnabled()) {
			tempLevel = Fourth;
		}else {
			tempLevel = Third;
		}
		
		if (_level == Third || ZeuS.subClassCumulativeIsEnabled())
		{
			if (player.getRace() != Race.KAMAEL)
			{
				...
				{
					case ELF:
						subclasses.removeAll(getSet(Race.DARK_ELF, tempLevel));
						break;
					case DARK_ELF:
						subclasses.removeAll(getSet(Race.ELF, tempLevel));
						break;
				}
				...
				subclasses.removeAll(getSet(Race.KAMAEL, tempLevel));
			...
			{
				subclasses = getSet(Race.KAMAEL, tempLevel);
				subclasses.remove(this);
				...
				if ( !_cumulativeData.isEnabled() && (!player.getSubClasses().containsKey(2) || (player.getSubClasses().get(2).getLevel() < 75)))
				{
