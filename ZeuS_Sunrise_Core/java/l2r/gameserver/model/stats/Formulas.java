...
public final class Formulas
{
	...
	private static Map<Integer, HashMap<Integer, HashMap<String, String>>> PRIVATE_CHANCE = new HashMap<Integer, HashMap<Integer, HashMap<String, String>>>();
	...
	public static void getLoadNewData()
	{
		try
		{
			PRIVATE_CHANCE.clear();
		}
		catch (Exception var0)
		{
			// empty catch block
		}
		File dir = new File("./config/zeus/zeus_skill_chance.xml");
		try
		{
			...
			for (Node n = doc.getFirstChild(); n != null; n = n.getNextSibling())
			{
				...
				for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling())
				{
					...
					int IDSKILL = Integer.valueOf(d.getAttributes().getNamedItem("id").getNodeValue());
					int IdPath = Integer.valueOf(d.getAttributes().getNamedItem("path").getNodeValue());
					PRIVATE_CHANCE.put(IDSKILL, new HashMap<Integer, HashMap<String, String>>());
					PRIVATE_CHANCE.get(IDSKILL).put(IdPath, new HashMap<String, String>());
					for (Node b = d.getFirstChild(); b != null; b = b.getNextSibling())
					{
						...
						try
						{
							switch (b.getAttributes().getNamedItem("name").getNodeValue().toLowerCase())
							{
								case "pvp_chance":
								{
									PRIVATE_CHANCE.get(IDSKILL).get(IdPath).put("PVP_CHANCE", b.getAttributes().getNamedItem("value").getNodeValue().toLowerCase());
									break;
								}
								case "oly_chance":
								{
									PRIVATE_CHANCE.get(IDSKILL).get(IdPath).put("OLY_CHANCE", b.getAttributes().getNamedItem("value").getNodeValue().toLowerCase());
								}
							}
							continue;
						}
						...
						{
							_log.warn("Error loading ZeuS Engine Formulas->" + aaa.getMessage() + "\n");
						}
					}
				}
			}
		...
		{
			_log.warn("Error loading ZeuS Engine Formulas->" + a.getMessage() + "\n");
		}
		_log.warn("Loading " + PRIVATE_CHANCE.size() + " skill from ZeuS Engine Formula System");
	}
	
	private static boolean isZeuSEngineSkillChange(L2Character attacker, L2Character target, L2Skill Skill)
	{
		if (PRIVATE_CHANCE == null)
		{
			return false;
		}
		
		if (PRIVATE_CHANCE.size() == 0)
		{
			return false;
		}
		
		if (!(target instanceof L2PcInstance) || !(attacker instanceof L2PcInstance))
		{
			return false;
		}
		L2PcInstance player = (L2PcInstance) attacker;
		if (PRIVATE_CHANCE.containsKey(Skill.getId()))
		{
			// FastMap<String, String> SkillInfo = PRIVATE_CHANCE.get(Skill.getId());
			int IDSKILLLEVEL = Skill.getLevel();
			int idPath = 0;
			if (IDSKILLLEVEL < 100)
			{
				return false;
			}
			idPath = IDSKILLLEVEL / 100;
			if (PRIVATE_CHANCE.get(Skill.getId()).containsKey(idPath))
			{
				int CHANCE_PVP = Integer.valueOf(PRIVATE_CHANCE.get(Skill.getId()).get(idPath).get("PVP_CHANCE"));
				int CHANCE_OLY = Integer.valueOf(PRIVATE_CHANCE.get(Skill.getId()).get(idPath).get("OLY_CHANCE"));
				if ((player.isInOlympiad() && (CHANCE_OLY >= 0)) || (CHANCE_PVP >= 0))
				{
					return true;
				}
			}
	...
	private static boolean isEffective(L2Character _Attacker, L2Character _target, L2Skill skill)
	{
		Random aleatorio;
		...
		int CHANCE_PVP = Integer.valueOf(PRIVATE_CHANCE.get(skill.getId()).get(idPath).get("PVP_CHANCE"));
		int CHANCE_OLY = Integer.valueOf(PRIVATE_CHANCE.get(skill.getId()).get(idPath).get("OLY_CHANCE"));
		L2PcInstance attacker = (L2PcInstance) _Attacker;
		L2PcInstance target = (L2PcInstance) _target;
		double vuln = Formulas.calcSkillTraitVulnerability(0.0, target, skill);
		double prof = Formulas.calcSkillTraitProficiency(0.0, attacker, target, skill);
		double lvlBonusMod = Formulas.calcLvlBonusMod(attacker, target, skill);
		double dyeinfo = skill.getBasicProperty().calcBonus(target);
		int Resistencia = (int) (vuln * -1.0) * 10;
		Resistencia += (int) lvlBonusMod * 1000;
		Resistencia += (int) dyeinfo * 1000;
		Resistencia /= 1000;
		boolean sendData = false;
		if (sendData)
		{
			Vector<String> Mensaje = new Vector<String>();
			Mensaje.add("------> vuln (" + String.valueOf(vuln) + ")");
			Mensaje.add("------> prof (" + String.valueOf(prof) + ")");
			Mensaje.add("------> lvlBonusMod (" + String.valueOf(lvlBonusMod) + ")");
			Mensaje.add("------> PVP_CHANCE (" + String.valueOf(CHANCE_PVP) + ")");
			Mensaje.add("------> OLY_CHANCE (" + String.valueOf(CHANCE_OLY) + ")");
			Mensaje.add("------> DyeInfo (" + String.valueOf(dyeinfo) + ")");
			Mensaje.add("------> 1 (" + skill.getAbnormalLvl() + ")");
			Mensaje.add("------> 2 (" + skill.getActivateRate() + ")");
			Mensaje.add("------> 3 (" + skill.getEffectAbnormalLvl() + ")");
			Mensaje.add("------> 4 (" + skill.getEffectLvl() + ")");
			Mensaje.add("------> 5 (" + skill.getLevel() + ")");
			Mensaje.add("------> 6 (" + skill.getMagicLevel() + ")");
			Mensaje.add("------> 7 (" + skill.getNegateLvl() + ")");
			Mensaje.add("------> Resistencia Natural (" + Resistencia + ")");
			for (String enviar : Mensaje)
			{
				attacker.sendMessage(enviar);
				_log.warn(enviar);
			}
		}
		if (attacker.isInOlympiad() && (CHANCE_OLY > 0))
		{
			if (CHANCE_OLY == 100)
			{
				return true;
			...
			int RandChar = 0;
			RandChar = aleatorio.nextInt(1000);
			if (RandChar < ((CHANCE_OLY - Resistencia) * 10))
			{
				return true;
			}
			return false;
		...
		if (CHANCE_PVP > 0)
		{
			...
			int RandChar = 0;
			RandChar = aleatorio.nextInt(1000);
			if (RandChar < ((CHANCE_PVP - Resistencia) * 10))
			{
				return true;
			}
			return false;
	...
	public static byte calcSkillReflect(L2Character target, L2Skill skill)
	{
		...
		switch (skill.getSkillType())
		{
			...
			case FEAR:
				// 200 --> Chance
				// 100 --> Cost
				// encuentra cual de los dos tipos de enchant es para trabajrlos despues
				// _log.warn("---->" + String.valueOf(skill.getLevel()));
			case ROOT:
			case STUN:
			case MUTE:
			case BLEED:
	...
	/** Allow Buff Lineal Cancelation, to the Last to the End */
	public static List<L2Effect> ZeuS_calcCancelStealEffects(L2Character activeChar, L2Character target, L2Skill skill, double power, boolean randomizeList)
	{
		// Resists.
		int negatedEffectCount = Rnd.get(1, skill.getMaxNegatedEffects()); // calcNegatedEffectCount(skill);
		
		final L2Effect[] _effects = target.getAllEffects();
		List<L2Effect> canceled = new ArrayList<>();
		
		List<L2Effect> ForCancel = new ArrayList<>();
		
		for (int Contador = _effects.length - 1; Contador > 0; Contador--)
		{
			ForCancel.add(_effects[Contador]);
		}
		
		// Cancel for Abnormals.
		if (skill.getNegateAbnormals() != null)
		{
			for (L2Effect eff : ForCancel)
			{
				if (eff == null)
				{
					continue;
		...
		{
			List<L2Effect> Temp_ForCancel = new ArrayList<>();
			for (L2Effect eff : ForCancel)
			{
				...
				Temp_ForCancel.add(eff);
			}
			
			int negated = 0;
			if (!Temp_ForCancel.isEmpty())
			{
				for (L2Effect e : Temp_ForCancel)
				{
	...
	public static int calcEffectAbnormalTime(L2Character caster, L2Character target, L2Effect effect)
	{
		...
		if ((caster != null) && (target != null) && effect.getSkill().isDebuff())
		{
			...
			if (caster.isDebug())
			{
				...
				caster.sendMessage("等級獎勵：" + lvlBonusMod);
				caster.sendMessage("elementMod: " + elementMod);
				caster.sendMessage("基礎時間：" + baseTime);
				caster.sendMessage("技能精通:" + skillMasteryAffected);
