...
public class RecipeController
{
	...
	private static class RecipeItemMaker implements Runnable
	{
		...
		public void run()
		{
			...
			{
				_target.sendMessage("目前已停用物品創建功能。");
				abort();
				return;
			}
			...
			if (Config.ALT_GAME_CREATION && !_activeMakers.containsKey(_player.getObjectId()))
			{
				if (_target != _player)
				{
					_target.sendMessage("製作已被中止");
					_player.sendMessage("製造已被中止");
				}
				else
				{
					_player.sendMessage("物品創建已中止");
				}
		...
		private void finishCrafting()
		{
			...
			else if (Rnd.get(100) < (_recipeList.getSuccessRate() + ZeuS.getPremiumCraftChance(_player, _recipeList.getSuccessRate())))
			{
				rewardPlayer(); // and immediately puts created item in its place
				updateMakeInfo(true);
				
				if (LeaderboardsConfigs.RANK_CRAFT_ENABLED && (_target != _player) && ((_recipeList.getSuccessRate() + ZeuS.getPremiumCraftChance(_player, _recipeList.getSuccessRate())) < 100))
				{
			...
			else
			{
				...
				else
				{
					...
					if (LeaderboardsConfigs.RANK_CRAFT_ENABLED && (_target != _player) && ((_recipeList.getSuccessRate() + ZeuS.getPremiumCraftChance(_player)) < 100))
					{
		...
		private void updateMakeInfo(boolean success)
		{
			...
			{
				_target.sendPacket(new RecipeShopItemInfo(_player, _recipeList.getId(), _target));
			}
		}
		...
		private void grabSomeItems()
		{
			...
			while ((grabItems > 0) && !_items.isEmpty())
			{
				...
				{
					_target.sendMessage("製造者 " + _player.getName() + " 使用了 " + count + " 個 " + item.getItemName());
				}
			}
		}
		...
		private boolean calculateStatUse(boolean isWait, boolean isReduce)
		{
			...
			for (L2RecipeStatInstance statUse : _recipeList.getStatUse())
			{
				...
				else
				{
					...
					_target.sendMessage("配方錯誤!!!，請將此情況告知您的GM。");
					ret = false;
					abort();
				}
			}
		...
		private void rewardPlayer()
		{
			...
			{
				if (Rnd.get(100) < (_recipeList.getRarity() + ZeuS.getPremium_MW_CraftChance(_player, _recipeList.getRarity())))
				{
					itemId = rareProdId;
