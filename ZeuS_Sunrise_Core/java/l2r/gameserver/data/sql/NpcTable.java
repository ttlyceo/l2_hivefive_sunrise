...
import gabriel.bypass.drops.DropListScope;
import gabriel.bypass.drops.GeneralDropItem;
import gabriel.bypass.drops.GroupedGeneralDropItem;
import gabriel.bypass.drops.IDropItem;
import l2r.Config;
...
public class NpcTable
{
	...
	private void fillNpcTable(ResultSet NpcData) throws Exception
	{
		...
		int expTemp = NpcData.getInt("exp");
		int spTemp = NpcData.getInt("sp");
		
		npcDat.set("OrirewardExp", expTemp);
		npcDat.set("OrirewardSp", spTemp);
		
		int Bonus = Config.getExpSpBonusZeuSEngine(level);
		if (Bonus > 0)
		{
			expTemp += (expTemp * Bonus) / 100;
			spTemp += (spTemp * Bonus) / 100;
		}
		
		npcDat.set("rewardExp", expTemp);
		npcDat.set("rewardSp", spTemp);
	...
	private boolean canLoadDrop(int IdDrop)
	{
		if (Config.ZEUS_BLOCK_DROP_ITEM != null)
		{
			if (Config.ZEUS_BLOCK_DROP_ITEM.size() > 0)
			{
				return !Config.ZEUS_BLOCK_DROP_ITEM.contains(IdDrop);
			}
		}
		return true;
	}
	...
	public int loadNpcsDrop(Connection con, int id, boolean isCustom)
	{
		...
		try (PreparedStatement ps = con.prepareStatement(query))
		{
			...
			try (ResultSet rs = ps.executeQuery())
			{
				...
				L2NpcTemplate npcDat = null;
				List<Integer> npcParsed = new ArrayList<>();
                while (rs.next())
				{
					if (!canLoadDrop(rs.getInt("itemId")))
					{
						continue;
					}
					...
					int tempChance = rs.getInt("chance");
					
					dropDat.setRealChance(tempChance);
					if (Config.ZEUS_DROP_CHANCE > 0)
					{
						if (!Config.ZEUS_DROP_CHANCE_BLACKLIST.contains(Integer.valueOf(rs.getInt("itemId"))))
						{
							int Porcent = Config.ZEUS_DROP_CHANCE;
							if (Config.ZEUS_DROP_CHANCE_ITEMS != null)
							{
								if (Config.ZEUS_DROP_CHANCE_ITEMS.containsKey(Integer.valueOf(rs.getInt("itemId"))))
								{
									Porcent = Config.ZEUS_DROP_CHANCE_ITEMS.get(Integer.valueOf(rs.getInt("itemId"))).intValue();
								}
							}
							Long tCh = Long.valueOf(tempChance);
							Long NewChanceLong = Long.valueOf((tCh.longValue() * Porcent) / 100L);
							int NewValue = 0;
							try
							{
								NewValue = NewChanceLong.intValue();
								NewValue += tempChance;
							}
							catch (Exception a)
							{
							...
							tempChance = NewValue > 1000000 ? 1000000 : NewValue;
						}
					}
					dropDat.setChance(tempChance);
					...
					if (Config.L2SOW_ALLOW_DROP_CLONE && (category > 0))
					{
						for (int _count = 1; _count <= Config.L2SOW_ALLOW_DROP_CLONE_AMOUNT; _count++)
						{
							npcDat.addDropData(dropDat, category + (200 * _count));
						}
					}
					npcParsed.add(mobId);
				}

				for (Integer integer : npcParsed) {
					npcDat = _npcs.get(integer);
					npcDat.generateChanceDrops();
				}
