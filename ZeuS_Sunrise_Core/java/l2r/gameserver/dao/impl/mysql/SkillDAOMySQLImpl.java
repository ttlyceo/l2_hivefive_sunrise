...
public class SkillDAOMySQLImpl implements SkillDAO
{
	...
	private static final String UPDATE_NORMAL = "UPDATE character_skills SET skill_level=? WHERE skill_id=? AND charId=? AND class_index=?";
	private static final String UPDATE_CUMULATIVE = "UPDATE character_skills SET skill_level=? WHERE skill_id=? AND charId=?";
	...
	private static final String DELETE_ONE_CUMULATIVE = "DELETE FROM character_skills WHERE skill_id=? AND charId=? AND class_index=0";
				...
				while (rs.next())
				{
					...
					if (Config.SKILL_CHECK_ENABLE)
					{
						...
						if (mustCheck)
						{
							...
							{
								if (player.getName().startsWith( ZeuS.getAIOPrefix() ) || _cumulativeData.isEnabled() )
								{
									continue;
								}
	...
	public void insert(L2PcInstance player, int classIndex, L2Skill skill)
	{
			...
			PreparedStatement ps = con.prepareStatement(INSERT))
		{
			...
			ps.setInt(4, ( _cumulativeData.isEnabled() ? 0 : classIndex )  );
			ps.execute();
		}
		catch (Exception e)
		{
	...
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement ps = con.prepareStatement( ( _cumulativeData.isEnabled() ? UPDATE_CUMULATIVE : UPDATE_NORMAL ) ))
		{
	...
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement ps = con.prepareStatement(( _cumulativeData.isEnabled() ? DELETE_ONE_CUMULATIVE : DELETE_ONE)))
		{
