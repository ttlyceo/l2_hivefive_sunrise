...
public final class TradeRequest extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		{
			player.sendMessage("您目前的存取等級已停用交易功能。");
		...
		{
			player.sendMessage("目前參與奧林匹亞競技的使用者不能接受或請求交易。");
			return;
		}
		
		if (ZeuS._isTradeRefusal(partner))
		{
			player.sendMessage(partner.getName() + "處於拒絕交易模式中");
			return;
		}
		
		if (!ZeuS.isActivePIN(player))
		{
			return;
		}

		if (!ZeuS.isActivePIN(partner))
		{
			player.sendMessage(partner.getName() + "需要輸入安全密碼");
			return;
		}		
		
		if(ZeuS.isBorrowActice(player)) {
			player.sendMessage( language.getInstance().getMsg(player).BORROW_SYSTEM_BLOCKED);
			return;
		}

		if(ZeuS.isBorrowActice(partner)) {
			player.sendMessage( language.getInstance().getMsg(player).BORROW_SYSTEM_THE_TARGET_$name_CANT_MAKE_TRADE.replace("$name", partner.getName()) );
			return;
		}		
		
		if (!ZeuS.canTradeFlag(player, partner))
		{
			return;
		}
		...
		{
			player.sendMessage("該玩家正在交易拒絕模式中。");
			return;
		}
