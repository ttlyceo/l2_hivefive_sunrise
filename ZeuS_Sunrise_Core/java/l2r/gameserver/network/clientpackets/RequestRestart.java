...
public final class RequestRestart extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		{
			player.sendMessage("無法在進行交易時重新啟動");
			sendPacket(RestartResponse.valueOf(false));
			return;
		}
		...
		{
			player.sendMessage("無法在混亂區域內進行重啟。");
			sendPacket(RestartResponse.valueOf(false));
			return;
		}
		...
		{
			player.sendMessage("無法在旗幟區域內重新啟動。");
			sendPacket(RestartResponse.valueOf(false));
			return;
		}
		...
		{
			player.sendMessage("由於防機器人保護，無法重新啟動。");
			sendPacket(RestartResponse.valueOf(false));
			return;
		}
		...
		{
			player.sendMessage("高等的力量不允許你離開這個事件。");
		...
		{
			player.sendMessage("您無法在奧林匹亞競技中重新開始。");
			player.sendPacket(RestartResponse.valueOf(false));
			return;
		}
		...
		if (player.isFestivalParticipant())
		{
			...
			{
				player.sendMessage("您無法在參與節慶活動時重新啟動。");
				sendPacket(RestartResponse.valueOf(false));
				return;
			}
