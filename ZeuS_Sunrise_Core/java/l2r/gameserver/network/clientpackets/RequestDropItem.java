...
public final class RequestDropItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		// if ((item == null) || (_count == 0) || !activeChar.validateItemManipulation(_objectId, "drop") || (!Config.ALLOW_DISCARDITEM && !activeChar.canOverrideCond(PcCondOverride.DROP_ALL_ITEMS)) || (!item.isDropable() && !(activeChar.canOverrideCond(PcCondOverride.DROP_ALL_ITEMS) &&
		// Config.GM_TRADE_RESTRICTED_ITEMS)) || ((item.getItemType() == EtcItemType.PET_COLLAR) && activeChar.havePetInvItems()) || activeChar.isInsideZone(ZoneIdType.NO_ITEM_DROP))
		if (((item == null) || (_count == 0) || !activeChar.validateItemManipulation(_objectId, "drop") || (!Config.ALLOW_DISCARDITEM && !activeChar.canOverrideCond(PcCondOverride.DROP_ALL_ITEMS)) || (!item.isDropable() && (!activeChar.canOverrideCond(PcCondOverride.DROP_ALL_ITEMS) || !Config.GM_TRADE_RESTRICTED_ITEMS)) || ((item.getItemType() == EtcItemType.PET_COLLAR) && activeChar.havePetInvItems()) || activeChar.isInsideZone(ZoneIdType.NO_ITEM_DROP)) && (!Config.ZEUS_DISCARD_ITEM || (Config.ZEUS_DISCARD_ITEM && !Config.ZEUS_DISCARD_ITEMS_ID.contains(item.getItem().getId()))))
		{
		...
		{
			activeChar.sendMessage("你無法在監獄中丟棄物品。");
			return;
		}
		...
		{
			activeChar.sendMessage("您的存取等級已停用交易功能。");
		...
		{
			activeChar.sendMessage("您無法使用此物品。");
			return;
		}
