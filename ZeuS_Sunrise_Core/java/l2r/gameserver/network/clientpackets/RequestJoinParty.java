	...
	protected void runImpl()
	{
        ...
        if(target.isPhantom()){
            if(target.isInStoreMode()){
                requestor.sendMessage("玩家處於離線模式。");
                return;
            }
        }else{
		...
		if(ZeuS.isInPvPInstance(requestor, false) || ZeuS.isInPvPInstance(target, false)){
			...
			{
				requestor.sendMessage("您無法邀請此玩家加入隊伍。他或您正在PvP副本區域中。");
				return;
			}
			
			if(ZeuS.isInPvPInstanceSoloClan(target)){
				if(requestor.getClan() != target.getClan()){
					requestor.sendMessage("只有來自同一個血盟的玩家才能在這個區域內加入隊伍。");
					return;
				}
			}
		}
		...
		else
		{
			...
			{
				requestor.sendMessage("當你在次元裂縫中時，你不能邀請玩家。");
			}
			else
			{
