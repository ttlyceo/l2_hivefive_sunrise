...
public final class Logout extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		{
			player.sendMessage("您無法在混亂區域內登出。");
			player.sendPacket(ActionFailed.STATIC_PACKET);
			return;
		}
		...
		{
			player.sendMessage("您無法在旗幟區域內登出。");
			player.sendPacket(ActionFailed.STATIC_PACKET);
			return;
		}
		...
		{
			player.sendMessage("您因防機器人保護無法登出。");
			player.sendPacket(ActionFailed.STATIC_PACKET);
			return;
		}
		...
		{
			player.sendMessage("您無法在奧林匹亞競技中登出。");
			player.sendPacket(ActionFailed.STATIC_PACKET);
			return;
		}
		...
		if (player.isFestivalParticipant())
		{
			...
			{
				player.sendMessage("當你正在參與節慶活動時，無法登出遊戲。");
				player.sendPacket(ActionFailed.STATIC_PACKET);
				return;
			}
