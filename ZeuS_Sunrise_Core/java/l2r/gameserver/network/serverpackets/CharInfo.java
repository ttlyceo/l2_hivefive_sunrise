...
public class CharInfo extends L2GameServerPacket
{
	...
	protected final void writeImpl()
	{
		...
		try
		{
			if (_activeChar.getEventInfo().hasAntifeedProtection() || _activeChar.hasAntiFeed() || _activeChar.isTransformarCHAR())
			{
				antifeed = true;
			}
			else
			...
			transform = _activeChar.isTransformarCHAR();
		}
		...
		if (isInvisible())
		{
			...
			{
				if(activeChar.getAccessLevel().getLevel() == 8) {
					gmSeeInvis = true;
				}else {
					if(_activeChar.getAccessLevel().getLevel() == 8 && _activeChar.isZeuSGmVis) {
						gmSeeInvis = true;
					}else if(_activeChar.getAccessLevel().getLevel()<8){
						gmSeeInvis = true;
					}else {
						gmSeeInvis = false;
					}
		...
		if (_activeChar.getPoly().isMorphed())
		{
			...
			if (template != null)
			{
				...
				writeD(-1); // High Five NPCString ID
				boolean isMasterInvis = _activeChar.getAccessLevel().getLevel() == 8 && !_activeChar.isZeuSGmVis;
				writeS(gmSeeInvis ? (isMasterInvis ? "[隱形狀態]" : "[隱形狀態]") : ("[" + _activeChar.getLevel() + "]" + _activeChar.getAppearance().getVisibleTitle()));
		...
		else
		{
			...
			writeD(_vehicleId);
			writeD(_objId);
			DressmeCommand.loadDressme(this._activeChar);
			L2PcTemplate TC = _activeChar.getAntifeedTemplate();
			if (TC == null)
			{
				transform = false;
			}
			...
			writeS(_activeChar.getAppearance().getVisibleName());
			writeD(!transform ? _activeChar.getRace().ordinal() : TC.getRace().ordinal());
			writeD(_activeChar.getAppearance().getSex() ? 1 : 0);
			writeD(!transform ? _activeChar.getBaseClass() : TC.getClassId().getId());
			
			boolean canSeeDressmeFromOthers = ZeuS.canSeeOthersDressme(getClient().getActiveChar());
			
			for (int slot : getPaperdollOrder())
			{
				int td = -100;
				if(canSeeDressmeFromOthers) {
					td = DressmeCommand.getVirtualSlot(_activeChar, slot);
				}
				writeD(((td == -100) ? _activeChar.getInventory().getPaperdollItemDisplayId(slot) : td));
			...
			if (transform)
			{
				...
				{
					writeF(TC.getFCollisionRadiusFemale());
					writeF(TC.getFCollisionHeightFemale());
				}
				else
				{
					writeF(TC.getfCollisionRadius());
					writeF(TC.getfCollisionHeight());
				}
			}
			else
			{
			...
			writeS(gmSeeInvis ? "Invisible" : ("[" + _activeChar.getLevel() + "]" + _activeChar.getAppearance().getVisibleTitle()));
			...
			int Recom = _activeChar.getRecomHave();
			...
			writeD(!transform ? _activeChar.getClassId().getId() : TC.getClassId().getId());
			writeD(0x00); // ?
