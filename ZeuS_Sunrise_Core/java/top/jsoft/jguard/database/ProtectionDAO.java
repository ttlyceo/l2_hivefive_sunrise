...
public class ProtectionDAO {
    ...
    public void storeHWID(final L2GameClient client, final boolean founded) {
        Connection con = null;
        PreparedStatement statement = null;
        if (founded) {
            try {
    ...
    public void storeHWIDBanned(final L2GameClient client, final String comment, final BanType banType) {
        Connection con = null;
        PreparedStatement statement = null;
        try {
            con = L2DatabaseFactory.getInstance().getConnection();
            ...
            statement.setString(2, comment);
            statement.setString(3, banType.toString());
            statement.setString(4, banType == BanType.ACCOUNT_BAN ? client.getLogin() : client.getActiveChar() != null ? client.getActiveChar().getName() : "N/A");
            statement.execute();
        } catch (final SQLException e) {
            LOGGER.error("Store error storeHWIDBanned(Class, String, Class)", e);
        } finally {
