...
import ZeuS.dressme.dress_player;
...
public class v_Dressme extends dressme{
	...
	private static String getMain(L2PcInstance player){
		...
		int IDSLOT[] = {Inventory.PAPERDOLL_LHAND,Inventory.PAPERDOLL_RHAND,Inventory.PAPERDOLL_CHEST,Inventory.PAPERDOLL_LEGS, Inventory.PAPERDOLL_GLOVES, Inventory.PAPERDOLL_FEET, Inventory.PAPERDOLL_CLOAK, Inventory.PAPERDOLL_HAIR, Inventory.PAPERDOLL_HAIR2};
		...
		for(_dressItem ItemDressme : DRESSME_ITEMS.get(Part)){
			...
			switch(IdSlot){
				...
				case 12:
					...
					break;						
				case 2: // HAIR
					if(L2Item.SLOT_HAIR != ItD.getItem().getBodyPart()){
						continue;
					}
					break;
				
				case 3: // HAIR2
					if(L2Item.SLOT_HAIR2 != ItD.getItem().getBodyPart()){
						continue;
					}
					break;
			}
		...
		if(parm1.equals("0")){
			...
			switch (idSlot) {
				...
				case Inventory.PAPERDOLL_HAIR:
					getHTML_DressmeItem(player, "HAIR", Pag, idSlot, false);
					break;
	...
	public static void removeItemFromDressme(L2PcInstance player) {
		if (BUYING_VAR == null || !BUYING_VAR.containsKey(player.getObjectId())) {
			return;
		}
		
		v_dressme_buy_var _tmp = BUYING_VAR.get(player.getObjectId());
		int partIndex = _tmp.getLoc();  // 例如 Inventory.PAPERDOLL_HAIR 或其他 slot
		
		int currentDressId = SEE_DRESSME_SHOW.get(player.getObjectId());
		dress_player dressPlayer = DRESSME_PLAYER.get(player.getObjectId());

		// 處理頭飾特殊邏輯
		if (partIndex == Inventory.PAPERDOLL_HAIR) {
			dressPlayer.getDressme(currentDressId).setHood1(0);
			dressPlayer.updateIsTheSame(currentDressId);
		} else if (partIndex == Inventory.PAPERDOLL_HAIR2) {
			dressPlayer.getDressme(currentDressId).setHood2(0);
			dressPlayer.updateIsTheSame(currentDressId);
		} else {
			dressPlayer.setIdDressmeToPart(partIndex, 0, currentDressId);
		}
