...
public class v_FlagFinder{
	...
	private static String mainHtml(L2PcInstance player, String Params, String BypassAnterior){
		...
		Requerimientos.add("僅限貴族使用," + String.valueOf(general.FLAG_FINDER_CAN_NOBLE));
		Requerimientos.add("使用者最低等級限制," + String.valueOf(general.FLAG_FINDER_LVL));
		Requerimientos.add("目標最低等級限制," + String.valueOf(general.FLAG_PVP_PK_LVL_MIN));
		Requerimientos.add("目標最低等級限制," + String.valueOf(general.FLAG_FINDER_MIN_PVP_FROM_TARGET));
		Requerimientos.add("可在「戰鬥狀態」下使用," + String.valueOf(general.FLAG_FINDER_CAN_USE_FLAG));
		Requerimientos.add("可在「紅人狀態（PK）」下使用," + String.valueOf(general.FLAG_FINDER_CAN_USE_PK));
		Requerimientos.add("優先尋找紅人（PK）," + String.valueOf(general.FLAG_FINDER_PK_PRIORITY));
