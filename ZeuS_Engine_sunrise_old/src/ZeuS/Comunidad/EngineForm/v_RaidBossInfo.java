...
public class v_RaidBossInfo{
	...
	private static String getInfoNPC(L2PcInstance player, int idMonster, String Sort, int idPagina){
		...
		if(Monster == null){
			central.msgbox("這個NPC有問題。我們無法載入NPC / 怪物的資訊。", player);
	...
	public static void _loadRaidInfo(){
		...
		String Qry = "SELECT boss_id, loc_x, loc_y, loc_z, " +
             "IF(respawn_time !=0, FROM_UNIXTIME(ROUND(respawn_time/1000, 0)), '0'), " +
             "killedBy " +
			...
			ResultSet rss = psqry.executeQuery();
				while (rss.next()){
					try{
						...
						String Killer = rss.getString(6);
						raidInfoClass t = new raidInfoClass(Nombre,Level,XX,YY,ZZ,IDD,Respawn, Killer);
						INFO_RAIDBOSS.add(t);
	...
	private static String getGrillaRb(int pagina,String buscar, String sortType){
		...
		String retorno = "<table width=720 border=0 cellpadding=0 cellspacing=0><tr><td>"+
				"<button value=\"狩獵首領名稱\" action=\""+ ByPass_NAME +"\" width=340 height=16 back=\"L2UI_CH3.HorizontalSliderBarCenter\" fore=\"L2UI_CH3.HorizontalSliderBarCenter\"></td><td>"+
				"<button value=\"等級\" action=\""+ ByPass_LVL +"\" width=60 height=16 back=\"L2UI_CH3.HorizontalSliderBarCenter\" fore=\"L2UI_CH3.HorizontalSliderBarCenter\"></td><td>"+
				"<button value=\"狀態\" action=\""+ ByPass_STATUS +"\" width=115 height=16 back=\"L2UI_CH3.HorizontalSliderBarCenter\" fore=\"L2UI_CH3.HorizontalSliderBarCenter\"></td><td>"+
				"<button value=\"最後擊殺者\" action=\"\" width=115 height=16 back=\"L2UI_CH3.HorizontalSliderBarCenter\" fore=\"L2UI_CH3.HorizontalSliderBarCenter\">"+
		...
		for(raidInfoClass TT : getRaidBossInfo(sortType)){
			...
			if(Contador >= Desde && Contador < Hasta){
				...
				String EstadoRb = RESPAWN.equalsIgnoreCase("0") ? "<font color=A9F5A9>存活</font>" :  "<font color=F78181>死亡</font>" ; 
				
				retorno += "<table width=720 border=0 cellpadding=0 cellspacing=0 bgcolor="+ Colores[Contador % 2] +" height=43><tr><td fixwidth=340 align=CENTER>"+
	            "<font color=81F7F3>"+ Nombre +"</font>"+ ( !RESPAWN.equalsIgnoreCase("0") ? "<br1>"+RESPAWN : "" ) + "</td>"+
	            "<td fixwidth=60 align=CENTER>"+ Level +"</td><td fixwidth=115 align=CENTER>"+
	            "<font color=A9F5A9>"+ EstadoRb +"</font></td>"+
				"<td fixwidth=115 align=CENTER><font color=F3F781>" + TT.getKilledBy() + "</font></td>"+
	            "<td fixwidth=90 align=CENTER><button value=\"查詢\" action=\""+ Bypass.replace("%IDNPC%", BOSSID) +"\" width=90 height=38 back=\"L2UI_CT1.Button_DF_Calculator_Over\" fore=\"L2UI_CT1.Button_DF_Calculator\">"+
	            "</td></tr></table>";				
			}
...
//			Retorno = getMonsterDropList(Integer.parseInt(Buscar), pagina, player, pagina);
	...
	private static String getDroplistWindows(L2PcInstance player, int pagina, int idRB, String SortType) {
		...
		String DropData = getMonsterDropList(idRB, pagina, SortType);
	...
	public static void setStatusToRB(int idRB, boolean status, String SpawnTime){
		for(raidInfoClass TT : INFO_RAIDBOSS){
			if(TT.getId() == idRB){
				...
				String Consulta = "SELECT killedBy FROM raidboss_spawnlist WHERE boss_id=?";
				try (Connection con = L2DatabaseFactory.getInstance().getConnection();
					...
					try (ResultSet rs = ps.executeQuery()) {
						if (rs.next()) {
							String killer = rs.getString("killedBy");
							TT.setKilledBy(killer); // 你需要新增這個 setter
						}
					}
				} catch (SQLException e) {
					e.printStackTrace();
...
class raidInfoClass{
	...
	public raidInfoClass(String Name, int Level, int X, int Y, int Z, int BossID, String Respawn, String Killer){
		this.RB_NAME = Name;
		this.LOC_X = X;
		this.LOC_Y = Y;
		this.LOC_Z = Z;
	...
	public final String getKilledBy(){
		return this.RB_KILLER != null ? this.RB_KILLER : "-";
	}
