...
public class v_PartyFinder{
	...
	private static String mainHtml(L2PcInstance player, String params, String ByPassAnterior){
		...
		Requerimientos.add("僅限貴族使用," + String.valueOf(general.PARTY_FINDER_CAN_USE_ONLY_NOBLE));
		Requerimientos.add("使用功能的最低等級," + String.valueOf(general.PARTY_FINDER_CAN_USE_LVL));
		Requerimientos.add("可在持旗狀態下使用," + String.valueOf(general.PARTY_FINDER_CAN_USE_FLAG));
		Requerimientos.add("隊長必須為貴族," + String.valueOf(general.PARTY_FINDER_GO_LEADER_NOBLE));
		Requerimientos.add("隊長為紅人狀態," + String.valueOf(general.PARTY_FINDER_GO_LEADER_FLAGPK));
