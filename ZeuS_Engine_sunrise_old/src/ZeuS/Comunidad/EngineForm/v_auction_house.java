...
public class v_auction_house {
	...
	private static void getInfoItemWindows(L2PcInstance player, int idObjecto){
       ...
       "<img src=\"L2UI_CT1.Gauge_DF_Attribute_Earth\" width=%WIDTH% height=10></td></tr></table></td>";		
		...
		html.replace("%IS_ONLINE%", (itemSel.isPlayerOnline() ? "上線中" : "未上線"));
		...
		if(player.getObjectId() == itemSel.getPlayerID()){
            ...
            "<button value=\"取消販賣\" action=\""+ BypassCancel +"\" width=150 height=34 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td></tr></table>";
		...
		if(!isOwner){
			if(opera.haveItem(player, itemSel.getIDItemRequested(), itemSel.getQuantityRequest(), false)){
	            ...
	            "<font color=LEVEL>購買數量</font>"+ (cantidad==1 ? "<br><font name=hs12>1</font>" : "<edit type=number var=\"txtQuantity\" width=120>") +"<br>"+
				"<button value=\"立即購買\" action=\""+ (cantidad==1 ? ByPassBuyItNow_1 : ByPassBuyItNow ) +"\" width=150 height=34 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td></tr></table>";			
	...
	private static void createAuctionWithItem(L2PcInstance player, int idObject, int idItemRequest, Long Cantidad_a_Vender, Long ValorPorCadaItem_A_Vender){
		...
		if (object instanceof L2ItemInstance){
			...
			central.msgbox("拍賣創建過程失敗。請檢查。", player);
		}
	}
		...
		for( InfoAuction p : SortedData(player)){
			...
			if(Grabar){
				...
				if(Contador >= desde && Contador <hasta){
					...
					String Nombre = (p.isSell() ? "(販售) " : "(購買) ") +   (p.getItemEnchant()>0 ? "<font color=5D6C5E>+"+ String.valueOf( p.getItemEnchant() ) + "</font> " : "" ) + ( opera.isMasterWorkItem(p.getIdItem()) ? "<font color=LEVEL>" +  p.getItemName() + "</font>" : p.getItemName());
