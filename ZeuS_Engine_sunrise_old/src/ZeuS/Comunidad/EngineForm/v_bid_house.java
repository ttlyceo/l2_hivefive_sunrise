...
public class v_bid_house {
	...
	private static void getInfoItemWindows(L2PcInstance player, int idObjecto, int seccion){
       ...
       "<img src=\"L2UI_CT1.Gauge_DF_Attribute_Earth\" width=%WIDTH% height=10></td></tr></table></td>";		
		...
		if(istheFirtsBidder || isBidder){
			...
			"我的競標: " + opera.getFormatNumbers( itemSel.getAllBiddersOrderByBid().get(player.getObjectId()).getBid()) + "<br1>" +
            "<button value=\"取消我的競標\" action=\""+ ByPassCancel.replace("%ID_OBJECT%", String.valueOf(idObjecto)) +"\" width=150 height=34 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td></tr></table><br>";
		...
		if(player.getObjectId() == itemSel.getPlayerID()){
            ...
            "<button value=\"取消競標\" action=\""+ BypassCancel +"\" width=150 height=34 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td></tr></table>";
			isOwner= true;
		}
		...
		if(!isOwner){
			if(opera.haveItem(player, itemSel.getIDItemRequested(), itemSel.getQuantityRequest(), false)){
	            ...
	            "<font color=LEVEL>你的出價<br1>(目前出價: "+ opera.getFormatNumbers(itemSel.getActualBid() ) +")</font>"+ "<edit type=number var=\"txtBid\" width=120><br>"+
				"<button value=\"出價\" action=\""+ ByPassBuyItNow +"\" width=150 height=34 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td></tr></table>";			
				...
				OwnerData += "<table width=264 background=L2UI_CT1.Windows_DF_TooltipBG cellspacing=2 cellpadding=2 border=0><tr><td fixwidth=264 align=CENTER>"+
				"<font color=FAF991>你的物品</font><br>"+
	            "<font color=B8FA91>"+ itemSel.getNameItemToRequest() +"</font><br1>"+
	            "<font color=91DEFA>"+ itemHave +"</font><br>"+
	           "</td></tr></table>";
			}else{
				OwnerData += "<table width=264 background=L2UI_CT1.Windows_DF_TooltipBG cellspacing=2 cellpadding=2 border=0><tr><td fixwidth=264 align=CENTER>"+
	            "<font color=FAF991>你的物品</font><br>你沒有所需的道具</td></tr></table>";
			}
		}
	...
	private static void createAuctionWithItem(L2PcInstance player, int idObject, int idItemRequest, Long Cantidad_a_Vender, Long ValorPorCadaItem_A_Vender){
		...
		if (object instanceof L2ItemInstance){
			...
			central.msgbox("競標系統創建過程失敗。請檢查。", player);
		}
	}
	...
	public static void _ByPass(L2PcInstance player, String params){
		...
		if(params==null){
			central.msgbox("輸入錯誤，請再試一次。", player);
			return;
		}
