...
public class profession {
	//		<button value="我需要強大的武器" action="link zeusPF;Weapon;0;0;0;0" width=200 height=32 back="L2UI_CT1.OlympiadWnd_DF_Apply" fore="L2UI_CT1.OlympiadWnd_DF_Apply">
	...
	public static void showProfesionWindows(L2PcInstance player){
		...
		String imageLogo = opera.getImageLogo(player);
		String HTML_R = html.getHtml();
		HTML_R = HTML_R.replace("%SERVER_LOGO%", imageLogo).replace("%CLASSES%", Clases).replace("%COST%", (Cost.length()>0 ? "" + Cost : ""));
		central.sendHtml(player, HTML_R, true);	
	}
}
