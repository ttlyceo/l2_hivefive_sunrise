...
public class menu {
	...
	private static String getConfigRaidbossInfo(L2PcInstance player, String idBox){
		...
		MAIN_HTML += setFilaBoolean("狩獵首領 傳送", general.RAIDBOSS_INFO_TELEPORT , inSeccion, "119");
		
		MAIN_HTML += setFilaBoolean("狩獵首領 觀察模式", general.RAIDBOSS_OBSERVE_MODE , inSeccion, "595");
		
		MAIN_HTML += setFilaBoolean("只有貴族玩家可以使用 狩獵首領 直接傳送功能。", general.RAIDBOSS_INFO_NOBLE , inSeccion, "120");

		MAIN_HTML += setFilaModif("首領傳送費用", "", inSeccion, "117");

		if(idBox.equals("117")){
			MAIN_HTML += getEdititem("首領傳送費用",general.RAIDBOSS_INFO_TELEPORT_PRICE ,inSeccion,idBox);
		}

		MAIN_HTML += setFilaModif("顯示的首領數量", String.valueOf(general.RAIDBOSS_INFO_LISTA_X_HOJA),inSeccion,"118");
		if(idBox.equals("118")){
			MAIN_HTML += getEditBox("要顯示的首領數量（建議設定為 25）", idBox, inSeccion);
		}

		MAIN_HTML += setFilaModif("使用 首領 直接傳送功能所需的最低等級", String.valueOf(general.RAIDBOSS_INFO_LISTA_X_HOJA),inSeccion,"121");
		if(idBox.equals("121")){
			MAIN_HTML += getEditBox("使用首領直接傳送功能的最低等級限制", idBox, inSeccion);
		}

		MAIN_HTML += setFilaModif("首領傳送黑名單（無法從首領資訊面板傳送的首領清單）", "", inSeccion, "351");
		if(idBox.equals("351")){
			MAIN_HTML += getEditListaNPC_VectorInteger("首領傳送黑名單",general.RAIDBOSS_ID_MOB_NO_TELEPORT ,inSeccion,idBox);
		}
	...
	private static String getConfigColor(L2PcInstance player, String idBox){
		...
		String MAIN_HTML = "<html><title>"+general.TITULO_NPC()+"</title><body><center>";
		MAIN_HTML += central.LineaDivisora(2)+central.headFormat("玩家名稱顏色設定")+ central.LineaDivisora(1);
			...
			switch(evento)/*Determinado despues de setConfig1*/{
				...
				case "setValor"://eventParam1 => ID EN BD
					...
					if(eventParam1.equals("1")){//ID BD NPC ZEUS
						if(Integer.valueOf(eventParam2)==general.ID_NPC_CH){
							central.msgbox("錯誤的 NPC ID。", player);
							return getConfigNPCZeuS(player, eventParam1);
						}
						...
						if((TempoVal<0) || (TempoVal>100)){
							central.msgbox("將數字放在1和100之間", player);
							return getConfigNPCZeuS(player, eventParam1);
						}
					}
				...
				case "setProducto":
					...
					if(!opera.isNumeric(eventParam1) || !opera.isNumeric(eventParam2))/*id item - cantidad*/{
						central.msgbox("Item的id和數量必須為數字", player);
					...
					if((central.getNombreITEMbyID(Integer.valueOf(eventParam1))==null) || central.getNombreITEMbyID(Integer.valueOf(eventParam1)).equals("none")){
						central.msgbox("輸入的項目ID不存在", player);
						return htmlShow(player,eventParam4/*Evento*/,eventParam3/*ID IN BD*/);
					}
				...
				case "setLista":
					...
					if(eventParam2.equals("317")){
						if(general.ANTIBOT_BORRAR_ITEM_ID.split(",").length>14){
							central.msgbox("你無法再新增更多 ID。", player);
							return htmlShow(player, eventParam3, eventParam2);
						}
					}
