...
public class OlympiadManagerLink implements IBypassHandler
{
	...
	public final boolean useBypass(String command, L2PcInstance activeChar, L2Character target)
	{
		...
		try
		{
				...
				switch (val)
				{
					case 0: // H5 match selection
						if (!OlympiadManager.getInstance().isRegistered(activeChar))
						{
							...
							html.replace("%1v1ClassMatch%", String.valueOf(OlympiadManager.getInstance().getRegisteredClassBased().size()));
							html.replace("%1v1NoClassMatch%", String.valueOf(OlympiadManager.getInstance().getRegisteredNonClassBased().size()));
							html.replace("%3v3TeamMatch%", String.valueOf(OlympiadManager.getInstance().getRegisteredTeamsBased().size()));
							activeChar.sendPacket(html);
						}
						else
						{
			...
			{
				if (ZeuS.showOlyBuff_CB(activeChar))
				{
					return true;
				}
