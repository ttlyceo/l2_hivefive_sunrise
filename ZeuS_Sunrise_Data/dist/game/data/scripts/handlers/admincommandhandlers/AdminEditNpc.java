...
public class AdminEditNpc implements IAdminCommandHandler
{
	...
	private void showEditDropData(L2PcInstance activeChar, int npcId, int itemId, int category)
	{
		...
		if (dropData != null)
		{
			...
			replyMSG.append(")</td><td></td></tr><tr><td>Real Chance (");
			replyMSG.append(dropData.getrealChance());
	...
	private int newChance(int idItem, int Chance)
	{
		int retorno = Chance;
		try
		{
			...
			{
				if (!Config.ZEUS_DROP_CHANCE_BLACKLIST.contains(idItem))
				{
					int Porcent = Config.ZEUS_DROP_CHANCE;
					
					if (Config.ZEUS_DROP_CHANCE_ITEMS != null)
					{
						if (Config.ZEUS_DROP_CHANCE_ITEMS.containsKey(idItem))
						{
							Porcent = Config.ZEUS_DROP_CHANCE_ITEMS.get(idItem);
						}
					}
					
					Long tCh = (long) Chance;
					Long NewChanceLong = ((tCh * Porcent) / 100);
					int NewValue = 0;
					try
					{
					...
					retorno = ( /* (tempChance + NewValue) */NewValue > 1000000 ? 1000000 : /* tempChance + NewValue */NewValue);
				}
			}
		}
		catch (Exception a)
		{
			_log.warn("Error al obtener el chance aumentado->" + a.getMessage());
		}
		return retorno;
	}
	...
	{
		// int tChance = newChance(itemId, chance);
			...
			try (PreparedStatement statement = con.prepareStatement("SELECT `mobId`, `itemId`, `min`, `max`, `category`, `chance` FROM `droplist` WHERE `mobId`=?"))
			{
				...
				try (ResultSet dropDataList = statement.executeQuery())
				{
					while (dropDataList.next())
					{
						...
						dropData.setRealChance(dropDataList.getInt("chance"));
						
						int tChance = newChance(dropDataList.getInt("itemId"), dropDataList.getInt("chance"));
				...
				try (PreparedStatement statement2 = con.prepareStatement("SELECT `mobId`, `itemId`, `min`, `max`, `category`, `chance` FROM `custom_droplist` WHERE `mobId`=?"))
				{
					...
					try (ResultSet dropDataList2 = statement2.executeQuery())
					{
						while (dropDataList2.next())
						{
							...
							dropData.setRealChance(dropDataList2.getInt("chance"));
							
							int tChance = newChance(dropDataList2.getInt("itemId"), dropDataList2.getInt("chance"));
