...
public class Q00350_EnhanceYourWeapon extends Quest implements IXmlReader
{
	...
	private static void levelCrystal(L2PcInstance player, SoulCrystal sc, L2Attackable mob)
	{
		...
		int ChanceFromCrystal = NPC_LEVELING_INFO.get(mob.getId()).get(sc.getLevel()).getChance();
		int PremiumChance = ZeuS.getPremiumSoulCrystalChance(player, ChanceFromCrystal);
		
		if (PremiumChance > 0)
		{
			if ((ChanceFromCrystal + PremiumChance) < 100)
			{
				ChanceFromCrystal += PremiumChance;
			}
			else if ((ChanceFromCrystal + PremiumChance) >= 100)
			{
				ChanceFromCrystal = 100;
			}
		}
		
		if (getRandom(100) <= ChanceFromCrystal)
		{
