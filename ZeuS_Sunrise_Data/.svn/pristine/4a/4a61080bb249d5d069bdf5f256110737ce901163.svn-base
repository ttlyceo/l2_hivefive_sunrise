	...
	public boolean onStart()
	{
		...
		if(target instanceof L2PcInstance){
			if(potionSystem.isEnabled()){
				t_amount = potionSystem.getMP(((L2PcInstance)target).getClassId().getId(), getSkill().getId(), ((L2PcInstance)target));
				oly_t_amount = potionSystem.getOlyMP(((L2PcInstance)target).getClassId().getId(), getSkill().getId());
